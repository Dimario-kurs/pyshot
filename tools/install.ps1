...
# установщик могли запустить из той же папки, куда ставим:
# копировать файл сам в себя нельзя, PowerShell на этом падает
...
$uninstallTarget = Join-Path $targetDir 'uninstall.ps1'
if ((Test-Path $uninstallSource) -and
        -not (Test-SamePath $uninstallSource $uninstallTarget)) {
