...
| **Перевод текста на снимке** | Кнопка в панели: текст распознаётся движком Windows офлайн, переводится и ложится поверх оригинала подложками. Повторное нажатие убирает перевод. Подробности — в разделе [Перевод](#перевод-текста-на-снимке). |
...
1. Запустите PyShot. В области уведомлений появится синий значок-плитка.
   Левый клик по нему открывает настройки, правый — всё меню.
...
прямоугольник сохраняется. Отсчёт стоит посреди того кадра, который сейчас
снимется, и растёт вместе с ним: на весь экран цифры крупные, на маленькой
области — маленькие, так что он не уедет на другой монитор и не закроет собой
весь снимок.

Поставьте таймер в положение **нет** — «Снять» сработает мгновенно. Рамка
хранится в настройках и переживает перезапуск.

По окончании отсчёта кадр открывается в **обычном редакторе**: можно порисовать
и выбрать «копировать», «сохранить» или «закрыть». Нужно, чтобы файл писался
сразу? Снимите галочку *Настройки → Таймер и запуск → «После снимка по таймеру
открывать редактор»*.
...
<img src="docs/translate.png" alt="Перевод текста прямо на снимке" width="820">

Кнопка с буквами **АЯ** в правой панели, под стрелкой отмены, либо
<kbd>Ctrl</kbd>+<kbd>T</kbd>. Первое нажатие переводит, второе — убирает
перевод и возвращает исходный кадр. Перевод попадает и в сохранённый файл,
и в буфер обмена.

Как это устроено:

1. **Распознавание — офлайн**, встроенным движком Windows. Никаких сторонних
   программ ставить не нужно, снимок никуда не отправляется. Нужны языковые
   пакеты Windows: *Параметры → Время и язык → Язык и регион*.
2. Строки целиком на чужом языке склеиваются в абзацы, чтобы перевод
   получался связным, а не порванным по строкам. А в строке, где чужие слова
   стоят вперемешку со своими, берутся только эти вставки, и подложка
   ложится ровно по ним.
3. **Перевод — через интернет.** Уходит только распознанный текст, не
   картинка. При первом нажатии программа спрашивает разрешение, а в
   настройках это отдельная галочка.
4. Поверх оригинала рисуются подложки: цвет фона и цвет букв берутся из
   самого снимка, размер шрифта подбирается так, чтобы перевод поместился.

### Смешанный текст

Если на снимке русский и английский вперемешку, программа разбирается сама.
Английский движок распознавания физически не умеет выводить кириллицу и
превращает русские слова в похожую латиницу, поэтому основой берётся русский
движок — он читает оба алфавита. Латинские слова при этом подставляются из
английского движка, который на них точнее. Переводчику явно указывается
чужой язык, иначе он видит кириллицу, объявляет весь текст русским и
возвращает его без изменений.

Русские слова остаются нетронутыми, английские становятся русскими —
буквально: подложка накрывает только английский участок, а русский текст
вокруг — это исходные пиксели снимка. Короткие вставки (`fork 7`, `Max`) и
имена вроде `TractPart_Project` или `config.json` не трогаются: переводить
там нечего, а подложка закрыла бы как раз то, что человек ищет.

### Сервисы перевода

| Сервис | Ключ | Заметки |
|---|---|---|
| **Google** | не нужен | работает сразу, по умолчанию; точка не документирована Google и может перестать отвечать |
| **DeepL** | нужен | лучшее качество для европейских языков |
| **Azure Translator** | нужен | 2 млн знаков в месяц бесплатно |

Ключ вписывается в *Настройках*, в разделе «Перевод».
...
| Таймер и запуск | задержка по умолчанию, показ отсчёта, редактор после снимка по таймеру, «Запускать вместе с Windows» |
...
├── translate.py           распознавание текста и обращение к переводчику
└── translation_layer.py   подложки с переводом: цвета, шрифт, отрисовка
...
Если что-то не работает у пользователя, пригодится встроенный отчёт:
`PyShot.exe --selftest` — покажет версию, доступность распознавания,
установленные языки OCR и путь к настройкам.
